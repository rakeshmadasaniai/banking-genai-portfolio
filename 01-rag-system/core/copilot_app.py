...
from features.voice_input import render_voice_input_helper, render_voice_input_toggle
...
def run_copilot_app() -> None:
    ...
    with st.sidebar:
        ...
        voice_enabled = render_voice_input_toggle()
    ...
    for message in st.session_state.messages:
        with st.chat_message(message["role"]):
            if message["role"] == "user":
                ...
                render_assistant_message(message, voice_enabled, accessibility.simplified_answers)

    voice_transcript = render_voice_input_helper(voice_enabled)
    ...
    if not question:
        with st.expander("README draft outline", expanded=False):
            st.markdown(
                """
                - **Overview**: what the copilot does and why grounded banking AI matters  
                - **Features**: model modes, uploads, accessibility, source-grounding  
                - **Architecture**: user query -> retriever -> knowledge base/uploaded docs -> model mode -> response  
                - **How to Run**: install requirements, set secrets, launch Streamlit  
                - **Model Modes**: OpenAI, Fine-Tuned, Auto  
                - **Screenshots**: home view, sidebar, cited answer, upload workflow  
                - **Evaluation**: test questions, groundedness checks, latency tracking  
                - **Roadmap**: OCR, stronger voice support, richer fine-tuned deployment path  
                """
            )
        return
    ...
    with st.chat_message("assistant"):
        render_assistant_message(assistant_message, voice_enabled, accessibility.simplified_answers)
